...
function M.new(config)
  ...
  self.adapters={}
  for _,name in ipairs({'vscode','finder','safari','terminal','figma','docker'}) do
  ...
  function self:checkpoint(id,captureCode)
    ...
    -- Code captures only on departure; manual/finish/COLD checkpoints must not
    -- create a first snapshot or overwrite a workflow with another desktop.
    if captureCode and self.vscode.checkpoint then U.try('VS Code snapshot',function() self.vscode:checkpoint(config.workflows[id]) end) end
    local live=self:discover(id)
    for _,item in ipairs(live) do
      if item.adapter~='vscode' then U.try(item.adapter..' checkpoint',function()
  ...
  function self:makeCold(id)
    ...
    -- This milestone is metadata-only COLD: no adapter close/quit/reopen hooks.
    state.lifecycle='COLD'; self.store:save(); U.log('INFO',id..' -> COLD (all resources preserved)'); return true
  end
  ...
  function self:switchTo(id,initial)
    ...
    local function begin()
      ...
      if old and not initial then
        self:checkpoint(old,true)
        if old~=id then
    ...
    if self.vscode.prepare then
      local prep;local resumed=false
      local function resume()
        if resumed or not prep:valid() then return end;resumed=true;prep:cancel();begin()
      end
      prep=Request.new(self.generation,function() return self.generation end,function(err) U.log('ERROR',err);resume() end)
      self.request=prep;prep:guard(function() self.vscode:prepare(prep,resume) end)
    else begin() end
    return true
  end
  ...
  function self:start()
    W.start()
    if self.vscode.start then self.vscode:start(function() return not self.switching and not self.stopped end) end
    ...
    U.log('INFO','Loaded '..#config.list..' workflows; starting GENERAL curation')
    hs.alert.show('WARP loaded',1)
    self:switchTo('general',true)
  end
  function self:stop()
