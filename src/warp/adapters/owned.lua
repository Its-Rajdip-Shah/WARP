-- Common lifecycle for apps with exclusive declared ownership (Figma/Docker UI).
...
function M.new(adapter,config)
  ...
  function self:discover(workflow)
    ...
    if #O.appOwners(config,adapter)>1 then return {} end -- shared UI is left untouched
    local out,counts={},{}
  ...
  function self:restore(workflow,state,ctx,done)
    ...
    if #O.appOwners(config,adapter)>1 then done(false,'shared '..adapter..' UI preserved; document ownership not configured'); return end
    local function restoreAll()
      local live=self:discover(workflow); local i=0
      local function nextWindow()
        i=i+1; local item=live[i]; if not item then done(true); return end
        local r=state.windows[W.key(adapter,item.identity)]
        if not r then
          -- A single recreated app window may reuse a single saved layout. Multiple unknown documents never get guessed.
          local saved={}; for _,v in pairs(state.windows) do if v.adapter==adapter then saved[#saved+1]=v end end
          if #live==1 and #saved==1 then r=U.copy(saved[1]) else
