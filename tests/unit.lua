...
test('configuration: bad paths, injected session, unsupported policies rejected',function()
  local b=basic(); b.one.terminal={tmuxSession='one',root='relative'}; assert(not C.validate(b))
...
test('invalid state schema/frames rejected',function()
  ...
  assert(not pcall(State.sanitize,{version=1,workflows={one={windows={a={adapter='terminal',identity='a',frame={x=0,y=0,w=-1,h=1}}}}}},cfg))
end)
